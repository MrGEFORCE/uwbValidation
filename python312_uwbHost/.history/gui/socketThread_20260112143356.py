        ...
        if self.runMode == self.ModeInstant:
            ...
            while True:
                ...
                if self.d[:const.cfg.MW_SIZE] == const.cfg.START_MAGIC_WORD:
                    ...
                    t = time.time()
                    received_magic = self.d[:const.cfg.MW_SIZE]
                    print(f"[连续接收] 理论包头值 (CTRL_HEAD): {const.cfg.CTRL_HEAD!r} 十六进制: {' '.join(f'{b:02x}' for b in const.cfg.CTRL_HEAD)}")
                    print(f"[连续接收] 接收到的包头值: {received_magic!r} 十六进制: {' '.join(f'{b:02x}' for b in received_magic)}")
                    print(f"[连续接收] 包头校验结果: {received_magic == const.cfg.START_MAGIC_WORD}")
                    self.main_thread.bytesData = self.d
                    self.cpltSig.emit(True)
                time.sleep(0.05)
            ...
            while True:
                ...
                if self.d[:const.cfg.MW_SIZE] == const.cfg.START_MAGIC_WORD:
                    ...
                    t = time.time()
                    received_magic = self.d[:const.cfg.MW_SIZE]
                    print(f"[完整帧接收] 理论包头值 (CTRL_HEAD): {const.cfg.CTRL_HEAD!r} 十六进制: {' '.join(f'{b:02x}' for b in const.cfg.CTRL_HEAD)}")
                    print(f"[完整帧接收] 接收到的包头值: {received_magic!r} 十六进制: {' '.join(f'{b:02x}' for b in received_magic)}")
                    print(f"[完整帧接收] 包头校验结果: {received_magic == const.cfg.START_MAGIC_WORD}")
